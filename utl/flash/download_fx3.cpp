		...
		cyusb_handle **h)
{
	...
	strcpy(progfile_p, "cyfxflashprog.img");
		...
		int            len)
{
	...
	while (len > 0) {
		...
		fprintf(stderr, "%3d\%\r", ((inital_len-len)*100)/inital_len);
		...
		int            len)
{
	...
	fprintf(stderr, "Verifying Page - %X\r\n", devAddr);
		...
		const char   *filename)
{
	...
	uint32_t original_filesize = filesize;

	printf ("Info: Writing firmware image to I2C EEPROM: %d\n", filesize);
	while (filesize != 0) {
	...
	free (fwBuf);
        fprintf(stderr, "Info: I2C programming completed\n");
	return 0;
}
		...
		const char   *filename)
{
	...
	if (r != 0) {
		...
		fprintf(stderr, "Info: SPI flash programming completed\n");
	}
